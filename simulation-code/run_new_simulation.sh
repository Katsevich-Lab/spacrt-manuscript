...
benchmark_memory_base=1 # Base memeory requirement for run_benchmark
...
sim_spec_fp=$(realpath "simulation-code/sim-spec-formal/${sim_name}.rds")
...
if [ ! -f "$output_dir/$output_filename" ]; then
  ...
  nextflow run katsevich-lab/simulatr-pipeline -r parallel-evaluation \
    ...
    --benchmark_memory_base $benchmark_memory_base \
    -profile $profile \

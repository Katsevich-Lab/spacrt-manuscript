...
######################
# plot the result for Type-I error; this code is for Figure 6
...
################################################################################
# The following code is for right-sided test; this code is for Figure 7
################################################################################
...
################################################################################
# The following code is to plot the left-sided test; this code is for Figure 6
################################################################################
...
################################################################################
# The following code is to plot the right-sided test; this code is for Figure 7
################################################################################
plot_gamma <- p_value_df_null |>
  dplyr::filter(gamma_0 == default_gamma &
                  sideness == "right" &
...
################################################################################
# The following code is to insert the p-values to bh/bonf; this code is for Table 5
################################################################################
...
reject_df <- p_value_df_null |>
  dplyr::select(method, sideness, p_value, gamma_0, beta_0, run_id) |>
  group_by(method, sideness, gamma_0, beta_0) |>
  # Calculate the partition group within each group
  dplyr::mutate(partition = ceiling(run_id / partition_size)) |>
  group_by(method, sideness, gamma_0, beta_0, partition) |>
  summarise(
    n_reject_bonf = sum(p.adjust(p_value, method = "bonferroni") <= alpha, na.rm = TRUE),
    n_reject_bh = sum(p.adjust(p_value, method = "BH") <= alpha, na.rm = TRUE)
  ) |>
  ungroup() |>
  # Now average over the 20 partitions within each group
  group_by(method, sideness, gamma_0, beta_0) |>
  summarise(
...
# loop over all rows of reject df
for (grid in 1:nrow(parameter_grid_null)) {

  # extract beta_0 and gamma_0
  beta_current <- parameter_grid_null$beta_0[grid]
  gamma_current <- parameter_grid_null$gamma_0[grid]

  # create the table
  reject_df |>
    tidyr::pivot_wider(names_from = "sideness",
                       values_from = c("n_reject_bonf", "n_reject_bh")) |>
    filter(beta_0 == beta_current, gamma_0 == gamma_current) |>
    ...
    knitr::kable(
      ...
      booktabs = TRUE,
      digits = 1,
      caption = sprintf("Number of rejections when $(\\beta_0,\\gamma_0,\\rho) = (%d, %d, 0)$.", beta_current, gamma_current),
      label = sprintf("simulation_rejection_beta_%d_gamma_%d", beta_current, gamma_current),
      col.names = NULL
    ) |>
    ...
    kableExtra::save_kable(sprintf("%s/simulation-num-rejection-beta-%d-gamma-%d.tex", plots_dir, beta_current, gamma_current))
}

# Plot the line plot
varying_gamma_rejection <- reject_df |>
  rename(BH = n_reject_bh,
         Bonferroni = n_reject_bonf) |>
  tidyr::pivot_longer(
    cols = c("BH", "Bonferroni"),
    names_to = "correction",
    values_to = "rejection"
  ) |>
  mutate(sideness = dplyr::if_else(sideness == "left", "Left-sided", "Right-sided")) |>
  group_by(method, sideness, correction, beta_0, gamma_0) |>
  mutate(rejection = rejection + runif(1, 1e-2,2e-2)) |>
  ungroup() |>
  filter(gamma_0 == -5) |>
  ggplot(aes(x = beta_0,
             y = rejection,
  ...
  theme(strip.text.x = element_text(margin = margin(0.04, 0, 0.04, 0, "cm"), size = 12),
        strip.text.y = element_text(margin = margin(0.04, 0.04, 0, 0.04, "cm"), size = 12),
        legend.position = "none",
...
y_title <- textGrob("Averaged number of rejections", rot = 90, gp = gpar(fontsize = 14))
...
# save the plot
ggsave(sprintf("%s/multiple_testing_rejection_plot.pdf", plots_dir),
       final_plot, height = 8, width = 5)


################################################################################
# The following code is to generate plot for comparing dCRT and spaCRT p-values (Figure 18, 19)
################################################################################
